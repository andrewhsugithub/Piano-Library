...
const dest =
  "C:\\Users\\Andrew\\OneDrive - 國立中正大學\\ccu cs\\程式設計(二)\\Final Project\\pianotranscription\\midi";
...
const expDL = async (scoreinfo: ScoreInfoHtml) => {
  ...
  //   // case "pdf": {
  //   //   // fileData = Buffer.from(await exportPDF(scoreinfo, scoreinfo.sheet));
  //   //   break;
  //   // }
  // }
  const type = "midi";
...
void (async () => {
  ...
  try {
    ...
    if (librescoreLink) {
      try {
        spinner.info(
          `${i18n("VIEW_IN_LIBRESCORE")()}: ${await librescoreLink}`
        );
