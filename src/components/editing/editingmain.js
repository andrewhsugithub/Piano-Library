...
const createAddNewMeasureButton = () => {
  ...
  for (
    let i = current_measure * BEAT_DIVITION * BEATS_PER_MEASURE + 1;
    i <= (current_measure + 1) * BEAT_DIVITION * BEATS_PER_MEASURE;
    i++
  ) {
    DrawVerticalLine(i);
  }
...
const AddNewMeasure = () => {
  ...
  for (
    let i = (current_measure + 1) * BEAT_DIVITION * BEATS_PER_MEASURE + 1;
    i <= (current_measure + 2) * BEAT_DIVITION * BEATS_PER_MEASURE;
    i++
  ) {
    DrawVerticalLine(i);
  }
...
const LoadMidiJSONtoObject = () => {
  const MIDIDATASTRING = document.getElementById("MIDIDATE").innerHTML;
  midi = JSON.parse(MIDIDATASTRING);
};
