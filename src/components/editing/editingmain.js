...
class PointAt {
    ...
    set_up() {
        ...
        const upperBound_result = upperBound(midi.sounds[this.cell_row - 1], this.cell_column);

        this.is_sound = midi.sounds[this.cell_row - 1][upperBound_result - 1] === undefined ? false : this.cell_column <= midi.sounds[this.cell_row - 1][upperBound_result - 1][1]
...
const CreateMenuBar = () => {
    ...
    window.addEventListener("scroll", (e) => {
        ...
        if (prevScrollpos > currentScrollPos) {
            ...
            document.getElementById("menu_bar").style.top = "-" + menu_bar.style.height;
        }
...
const CreateSaveButton = () => {
    ...
    button_p.style.fontSize = Math.ceil(window.innerHeight * (0.05)).toString() + "px";
...
const CreateModeViewer = () => {
    ...
    mode_p.style.fontSize = Math.ceil(window.innerHeight * (0.06)).toString() + "px";
...
const ResponseToWindowResize = () => {
    ...
    window.addEventListener("resize", (e) => {
        ...
        modeviewer.children[0].style.fontSize = Math.ceil(window.innerHeight * (0.06)).toString() + "px";

        menu_bar.style.height = (Math.ceil(window.innerHeight * (0.1))).toString() + "px";

        savebutton.children[0].style.fontSize = Math.ceil(window.innerHeight * (0.05)).toString() + "px";
    });
}
...
const CreateGridContainer = (measures) => {
    ...
    gridcontainer.style.width = (measures * BEAT_DIVITION * BEATS_PER_MEASURE * CELL_WIDTH).toString() + "px";
...
const DrawGrid = (measures) => {
    ...
    /* draw vertical lines */
    for (let i = 1; i <= midi.measures * BEAT_DIVITION * BEATS_PER_MEASURE; i++) DrawVerticalLine(i);
...
const DrawPanel = (measures) => {
    ...
    panel.style.width = (measures * BEAT_DIVITION * BEATS_PER_MEASURE * CELL_WIDTH).toString() + "px";
...
const DrawHorizontalLine = (y, measures) => {
    ...
    H_line.style.width = (measures * BEAT_DIVITION * BEATS_PER_MEASURE * CELL_WIDTH).toString() + "px";
...
const DrawVerticalLine = (x) => {
    ...
    if ((x - 1) % (BEAT_DIVITION * BEATS_PER_MEASURE) == 0) { // one measure
        ...
        p_number.innerHTML = ((x - 1) / (BEAT_DIVITION * BEATS_PER_MEASURE) + 1).toString();
...
const createAddNewMeasureButton = () => {
    ...
    editercontainer.style.width = (current_width + BEAT_DIVITION * BEATS_PER_MEASURE * CELL_WIDTH + 300).toString() + "px";
    gridcontainer.style.width = (current_width + BEAT_DIVITION * BEATS_PER_MEASURE * CELL_WIDTH).toString() + "px";
    panel.style.width = (current_width + BEAT_DIVITION * BEATS_PER_MEASURE * CELL_WIDTH).toString() + "px";

    for (const hl of h_line) {
        hl.style.width = (current_width + BEAT_DIVITION * BEATS_PER_MEASURE * CELL_WIDTH).toString() + "px";
    };

    for (let i = current_measure * BEAT_DIVITION * BEATS_PER_MEASURE + 1; i <= (current_measure + 1) * BEAT_DIVITION * BEATS_PER_MEASURE; i++) {
        DrawVerticalLine(i);
    }
    ...
    addnewmeasure_button_mask.style.width = (BEAT_DIVITION * BEATS_PER_MEASURE * CELL_WIDTH).toString() + "px";
    addnewmeasure_button_mask.style.height = (BAR_LENGTH + (MEASURE_NUMBER_HEIGHT * (0.5) + MEASURE_NUMBER_SPACE) * 2).toString() + "px";
    ...
    addnewmeasure_button_mask.style.top = (BAR_TOP - (MEASURE_NUMBER_HEIGHT * (0.5) + MEASURE_NUMBER_SPACE)).toString() + "px";
...
const AddNewMeasure = () => {
    ...
    editercontainer.style.width = (current_width + BEAT_DIVITION * BEATS_PER_MEASURE * CELL_WIDTH + 300).toString() + "px";
    soundscontainer.style.width = (current_width + BEAT_DIVITION * BEATS_PER_MEASURE * CELL_WIDTH).toString() + "px";
    gridcontainer.style.width = (current_width + BEAT_DIVITION * BEATS_PER_MEASURE * CELL_WIDTH).toString() + "px";
    panel.style.width = (current_width + BEAT_DIVITION * BEATS_PER_MEASURE * CELL_WIDTH).toString() + "px";
    ...
    for (let i = (current_measure + 1) * BEAT_DIVITION * BEATS_PER_MEASURE + 1; i <= (current_measure + 2) * BEAT_DIVITION * BEATS_PER_MEASURE; i++) {
        DrawVerticalLine(i);
    }
...
const CreateSoundsContainer = (measures) => {
    ...
    soundscontainer.style.width = (measures * BEAT_DIVITION * BEATS_PER_MEASURE * CELL_WIDTH).toString() + "px";
...
const CreateSoundDiv = (note, start, end, velocity = 60) => {
    ...
    range_bar.addEventListener("input", (e) => {
        ...
        midi.sounds[note - 1][new_sound.dataset.lookuptable_index][2] = parseInt(velocity);
    })
...
const ChangeSoundLength = (current_sound, mode, end, bound) => {
    ...
    const color = CalculateColor(midi.sounds[current_sound.note - 1][current_sound.index][2]);
    ...
    if (mode === "head") {
        ...
        /* style */
        current_sound_div.style.left = ((new_start - 1) * CELL_WIDTH).toString() + "px";
...
const isInPanel = (pageX, pageY) => {
    ...
    if (x_relate > 0 && x_relate < midi.measures * BEAT_DIVITION * BEATS_PER_MEASURE * CELL_WIDTH &&
...
const InsertNewSound = (note, start, end, velocity) => {
    ...
    midi.sounds[note - 1].sort(function(a, b) { return a[0] - b[0] });
}
...
const main = () => {
    ...
    let black_mask = document.getElementById("blackmask");
    black_mask.style.width = (2 * LEFT_MARGIN + midi.measures * BEAT_DIVITION * BEATS_PER_MEASURE * CELL_WIDTH).toString() + "px";
    black_mask.style.height = (2 * TOP_MARGIN + CELL_HEIGHT * 88).toString() + "px";

    document.getElementById("editer-container").style.width = (2 * LEFT_MARGIN + midi.measures * BEAT_DIVITION * BEATS_PER_MEASURE * CELL_WIDTH).toString() + "px";
